...
//                        instead of needing TRACK_CONFIRM fresh sightings.
      ...
      float PRE_CORNER_ZONE_MM   = 0.0;    // 0 = pre-corner swing off
      float PRE_CORNER_SWING_MM  = 0.0;
...
void resetLaneAlong() { laneAlongMm = 0.0; laneAlongEnc = readEncoder();
                        haveInnerPass = false; clearCrossCorner(); }
...
void updatePlanner() {
  ...
  if (!any) {
    if (cornerCount > 0 && laneAlongMm < POST_CORNER_BOOST_MM)
      ...
      target = (clockwiseMode ? 1.0f : -1.0f) * PRE_CORNER_SWING_MM;
  }
...
void planCornerExit() {
  ...
  if (nextStraightColor != VIS_NONE) {
    ...
    cornerExitCmd = (nextStraightColor == VIS_RED ? -1.0f : 1.0f) * CORNER_EXIT_BIAS_MM;
    ...
    Serial.println(inner ? F("INNER") : F("OUTER"));
  } else {
    cornerExitCmd = outer * CORNER_EXIT_MM;        // nothing seen: the default
  }
  turnFrontCmd = (inner && CORNER_ARC_ADAPT) ? TURN_FRONT_INNER_MM : TURN_FRONT_MM;
  turnLockCmd  = (inner && CORNER_ARC_ADAPT) ? TURN_LOCK_INNER     : TURN_LOCK_FRACTION;
}
...
// Turned OFF by default and settable from the Pi page (it is a param now, not
// a rebuild). It never fired before because this flag was false AND
// innerPillarNearCorner() returns false on the first line when it is.
       bool     USE_CORNER_MANEUVER   = false;
...
const uint16_t PARAM_VERSION        = 3;     // bump when ids are added/removed
